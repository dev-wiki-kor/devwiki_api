...
public interface TechArticleCommentEntityRepository extends JpaRepository<TechArticleCommentEntity, Long> {
    ...
    @Query("SELECT MAX(v.numForSort) from TechArticleCommentEntity v where v.articleId = :articleId and v.commentOrder = :commentOrder and v.level = :level")
    Long findMaxSortNumberForLevel(Long articleId, Long commentOrder, long level);

    @Modifying
    @Query("UPDATE TechArticleCommentEntity a SET a.numForSort = a.numForSort + 1 " +
            "WHERE a.articleId = :articleId AND a.commentOrder = :commentOrder AND a.numForSort >= :numForSort")
    void incrementNumForSort(@Param("articleId") Long articleId,
                             @Param("commentOrder") Long commentOrder,
                             @Param("numForSort") Long numForSort);
